...
void sendEventMessageWithRoute(UBOOL8 assigned, const char *ip, const char *mask, const char *gateway, const char *nameserver,UINT32 LEDControl, route_info_t *routeList, int routeNum, unsigned int lease_time)
#else
void sendEventMessageWithRoute(UBOOL8 assigned, const char *ip, const char *mask, const char *gateway, const char *nameserver,UINT32 LEDControl, route_info_t *routeList, int routeNum)
#endif
{
...
//#if defined(AEI_VDSL_CUSTOMER_TELUS)
#if defined(AEI_VDSL_CUSTOMER_DHCP_WAN_LEASETIME) //modify william 2011-11-29
            sendEventMessage(FALSE, NULL, NULL, NULL, NULL, 0, 0);
#else
...
static void AEI_handle_alarm(int sig)
{
    ...
    if (needArpDetectContinue) {
        if ((AEI_checkIfHasIP(client_config.interface) == 1)
            && !arpping(inet_addr(router_ip), inet_addr(local_ip), client_config.arp, client_config.interface)) {
...
#if defined(AEI_VDSL_CUSTOMER_DHCP_WAN_LEASETIME)						
                    sendEventMessage(FALSE, NULL, NULL, NULL, NULL, 1,0);
#else
                        sendEventMessage(FALSE, NULL, NULL, NULL, NULL, 1);
#endif
...
*/
            ...
            if (listen_mode == LISTEN_KERNEL) {
                if (get_packet(&packet, fd) < 0)
                    continue;
            } else {
                if (get_raw_packet(&packet, fd) < 0)
                    continue;
            }
            ...
            if (packet.xid != xid && *message != DHCPFORCERENEW ) 
	     {
            ...
            case REBINDING:
                if (*message == DHCPACK) {
                    ...
                    //run_script(&packet, ((state == REBINDING) ? "renew" : "bound"));
                    state = BOUND;
#if defined(AEI_VDSL_CUSTOMER_DHCPFORCERENEW) //add william 2011-11-18
					listen_mode = LISTEN_KERNEL;
					//listen_mode = LISTEN_RAW;
